...
//   - the sorter (via sorterEmergencyStop(), added Phase 12 - halts
//     in place rather than continuing a blocking move, since the
//     sorter is now non-blocking)
...
// Must be called frequently from loop() (Phase 12 addition). Polls
// PIN_EMERGENCY_STOP (if configured - defaults to -1/unconfigured, a
// no-op) and auto-triggers triggerEmergencyStop() if pressed. This is
// what makes e-stop NOT depend solely on a serial command arriving -
// per project spec section 15: "the software must not rely solely on
// Python to stop the robot."
void safetyUpdate();
