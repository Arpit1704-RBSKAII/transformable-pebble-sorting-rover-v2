...
// ============================================================
// TRANSFORMATION — TODO: CALIBRATE ON HARDWARE
// ============================================================
// Reuses the EXISTING SERVO_IDX_TRANSFORM slot from the servo section
// above (defined since Phase 2), rather than a separate dedicated
// servo. The repo's own transformation config file defined a second,
// independent TRANSFORM_SERVO_PIN (19) for a standalone Servo object -
// that would have created a real conflict (either two systems fighting
// over the same physical pin, or an unexplained second servo the
// original spec never mentioned - section 8 lists exactly ONE
// transformation servo). Reusing the existing slot avoids that
// entirely and matches how arm.cpp/gripper.cpp already share the same
// servos.h system rather than each owning a private Servo object.

#define TRANSFORM_ANGLE_STOW    90    // unfolded / sorting mode - TODO: CALIBRATE ON HARDWARE
#define TRANSFORM_ANGLE_FOLDED  20    // folded / compact rover mode - TODO: CALIBRATE ON HARDWARE
#define TRANSFORM_MOVE_TIMEOUT_MS 12000  // longer than arm/gripper - this is a bigger mechanical motion

// Optional limit switches confirming folded/unfolded positions.
// TODO: CONFIGURE IF WIRED - defaults to -1 (not present).
