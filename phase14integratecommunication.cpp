...
enum PendingAction {
  ...
  PENDING_ARM,
  PENDING_GRIPPER,
  PENDING_TRANSFORM  // Phase 13 addition
};
...
static void beginTransformAction(const char* label) {
  pending = PENDING_TRANSFORM;
  pendingLabel = label;
}
...
static void handleTransform(char* tokens[], int tokenCount) {
  // Phase 13: now actually implemented (previously always returned
  // ERROR,NOT_IMPLEMENTED as a placeholder).
  if (isEmergencyStopped()) {
    Serial.println("ERROR,EMERGENCY_STOP_ACTIVE");
    return;
  }
  ...
  if (strcmp(subcommand, "FOLD") == 0) {
    if (!foldTransform()) {
      // foldTransform() itself checks preconditions (arm home, gripper
      // closed, no active e-stop) and logs the specific reason to
      // Serial via [TRANSFORM] debug prints - the client only needs to
      // know it was rejected, not the raw internal check that failed.
      Serial.println("ERROR,TRANSFORM_PRECONDITIONS_FAILED");
      return;
    }
    beginTransformAction("TRANSFORM_FOLD");
    Serial.println("OK");
  } else if (strcmp(subcommand, "UNFOLD") == 0) {
    if (!unfoldTransform()) {
      Serial.println("ERROR,TRANSFORM_PRECONDITIONS_FAILED");
      return;
    }
    beginTransformAction("TRANSFORM_UNFOLD");
    Serial.println("OK");
  } else {
    Serial.println("ERROR,UNKNOWN_TRANSFORM_SUBCOMMAND");
  }
}
...
static void handleStatus() {
  ...
  //
  // Phase 14 addition: now also reports MOTORS/SORTER/TRANSFORM busy
  // state, not just ARM/GRIPPER - a status command should reflect
  // every subsystem, not just the two that existed when this was
  // first written in Phase 5.
  Serial.print("STATUS,ARM=");
  Serial.print((int)getArmPosition());
  Serial.print(",GRIPPER=");
  Serial.print((int)getGripperState());
  Serial.print(",MOTORS_BUSY=");
  Serial.print(motorsAreMoving() ? 1 : 0);
  Serial.print(",SORTER_BUSY=");
  Serial.print(isSorterBusy() ? 1 : 0);
  Serial.print(",TRANSFORM_BUSY=");
  Serial.print(isTransformBusy() ? 1 : 0);
  Serial.print(",ESTOP=");
...
void commUpdate() {
  ...
  if (pending == PENDING_ARM && isArmSettled()) {
    ...
    Serial.println(pendingLabel);
    pending = PENDING_NONE;
  } else if (pending == PENDING_TRANSFORM && !isTransformBusy()) {
    Serial.print("DONE,");
    Serial.println(pendingLabel);
    pending = PENDING_NONE;
  }
